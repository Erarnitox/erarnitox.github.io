...
EOF
  ...
  if [ -f "$INDEX_HTML_FILE" ]; then
    ...
    if grep -q "$HTML_DIR" "$INDEX_HTML_FILE"; then
      ...
      sed -i "/<!--ends here-->/i <div class=\"pub-item\"><img class=\"pub-image\" src=\"/res/bg.png\" alt=\"image\"><div><h2 class=\"pub-heading\">$HTML_TITLE</h2><p class=\"pub-description\">$HTML_DESC</p><a href=\"./$HTML_DIR\" class=\"pub-button\">Read</a></div></div>" "$INDEX_HTML_FILE"
